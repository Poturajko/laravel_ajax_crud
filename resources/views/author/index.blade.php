@extends('layouts.app')
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="card">

                    <div class="card-header">
                        <h4>
                            Author Data
                            <a href="#" data-bs-toggle="modal" data-bs-target="#AddAuthorModal"
                               class="btn btn-primary btn-sm float-end">Add Author</a>
                        </h4>
                    </div>

                    <div class="card-header">
                        <form action="{{route('authors.index')}}" method="GET">
                            @csrf
                            <div class="row justify-content-end">
                                <div class="col-4">
                                    <div class="form-check">
                                        <input class="form-check-input" name="order_by" type="checkbox" value="name-a-z" id="flexCheckDefault">
                                        <label class="form-check-label" for="flexCheckDefault">
                                            Name: A-Z
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" name="order_by" type="checkbox" value="name-z-a" id="flexCheckChecked">
                                        <label class="form-check-label" for="flexCheckChecked">
                                            Name: Z-A
                                        </label>
                                    </div>
                                </div>
                                <div class="col-4">
                                    <div class="input-group">
                                        <input type="search" name="search_name" class="form-control rounded" placeholder="Search" aria-label="Search"
                                               aria-describedby="search-addon" />
                                        <button type="submit" class="btn btn-primary">Search</button>

                                        <a href="{{route('authors.index')}}" class="btn btn-warning">Reset</a>
                                        <a href="{{route('index')}}" class="btn btn-success">Main Page</a>
                                    </div>
                                </div>
                            </div>
                        </form>
                    </div>

                    <div class="card-body">
                        <div class="table-responsive">

                        </div>
                    </div>

                    <div class="card-body">
                        <table class="table table-bordered">
                            <thead>
                            <tr>
                                <th>ID</th>
                                <th>Last Name</th>
                                <th>First Name</th>
                                <th>Middle Name</th>
                                <th>Edit</th>
                                <th>Delete</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($authors as $author)
                                <tr>
                                    <td>{{$author->id}}</td>
                                    <td>{{$author->last_name}}</td>
                                    <td>{{$author->first_name}}</td>
                                    <td>{{$author->middle_name}}</td>
                                    <td>
                                        <button type="button" data-bs-toggle="modal" data-bs-target="#EditAuthorModal"
                                                value="{{$author->id}}" class="edit_btn btn btn-success btn-sm">Edit
                                        </button>
                                    </td>
                                    <td>
                                        <form action="{{ route('authors.destroy', $author->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <input class="delete_btn btn btn-danger btn-sm" type="submit"
                                                   value="Delete">
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                        {{ $authors->links('components.paginate') }}
                    </div>

                </div>
            </div>
        </div>
    </div>

    @extends('author.add-modal')
    @extends('author.edit-modal')
@endsection

@section('scripts')
    <script>
        $(document).ready(function () {
            $(document).on('submit', '#AddAuthorForm', function (e) {
                e.preventDefault();

                let formData = new FormData($('#AddAuthorForm')[0])

                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    }
                });

                $.ajax({
                    type: "POST",
                    url: "{{route('authors.store')}}",
                    data: formData,
                    contentType: false,
                    processData: false,
                    success: function (response) {
                        if (response.status == 400) {
                            $('#save_errorList').html("");
                            $('#save_errorList').removeClass('d-none');
                            $.each(response.errors, function (key, err_value) {
                                $('#save_errorList').append('<li>' + err_value + '</li>')
                            })
                        } else if (response.status == 200) {
                            $('#save_errorList').html("");
                            $('#save_errorList').addClass('d-none');

                            $('#AddBookForm').find('input').val('')
                            $('#AddBookModal').modal('hide')

                            alert(response.message)
                        }
                    }
                })
            })
        })

        $(document).on('click', '.edit_btn', function (e) {
            e.preventDefault();

            let author_id = $(this).val()

            $('#EditAuthorModal').modal('show')

            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });

            $.ajax({
                type: 'GET',
                url: '/authors/' + author_id + '/edit',
                success: function (response) {
                    $('#edit_first_name').val(response.author.first_name)
                    $('#edit_last_name').val(response.author.last_name)
                    $('#edit_middle_name').val(response.author.middle_name)
                    $('#author_id').val(author_id)

                }
            })

        })

        $(document).on('submit', '#UpdateAuthorForm', function (e) {
            e.preventDefault();

            let id = $('#author_id').val()
            let EditFormData = new FormData($('#UpdateAuthorForm')[0])

            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });

            $.ajax({
                type: "POST",
                url: "/authors/" + id,
                data: EditFormData,
                contentType: false,
                processData: false,
                success: function (response) {
                    if (response.status == 400) {
                        $('#update_errorList').html("");
                        $('#update_errorList').removeClass('d-none');
                        $.each(response.errors, function (key, err_value) {
                            $('#update_errorList').append('<li>' + err_value + '</li>')
                        })
                    } else if (response.status == 200) {
                        $('#update_errorList').html("");
                        $('#update_errorList').addClass('d-none');

                        $('#EditAuthorModal').modal('hide');
                        alert(response.message);
                    }
                }
            })
        })
    </script>
@endsection

